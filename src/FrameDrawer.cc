...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    vector<cv::KeyPoint> vCurrentKeys, vCurrentKeysDynamic; // KeyPoints in current frame
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
{
    ...
    else if(nState==Tracking::OK)
    {
        ...
        s << "Maps: " << nMaps << ", KFs: " << nKFs << ", MPs: " << nMPs << ", Matches: " << mnTracked << ", FPSofVO: " << this->framePerSecond;
        if(mnTrackedVO>0)
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    pTracker->imDepth.copyTo(mDepth);
