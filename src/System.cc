    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //Initialize the PointCloud Mapping thread
    if(mSensor==STEREO || mSensor==IMU_STEREO || mSensor==RGBD || mSensor==IMU_RGBD)
    {
        ...
        node = fsSettings["PointCloudMapping"];
        bool savepcd = false;
        if(!node.empty())
        {
            savepcd = static_cast<int>(fsSettings["PointCloudMapping"]) != 0;
        }

        mpPointCloudMapping = new PointCloudMapping(resolution, meank, thresh, savepcd);
    ...
    node = fsSettings["UseDynamic"];
    if(!node.empty())
    {
        mpYoloDetector->mbYOLO = static_cast<int>(fsSettings["UseDynamic"]) != 0;
    }
    mptYoloDetector = new thread(&ORB_SLAM3::YoloDetector::Run, mpYoloDetector);
    ...
    //if(false) // TODO
    {
        ...
        if(mSensor==STEREO || mSensor==IMU_STEREO || mSensor==RGBD || mSensor==IMU_RGBD)
        {
            mpPointCloudMapping->SetViewer(mpViewer);   
        }
...
void System::Shutdown()
{
    ...
    if(mpPointCloudMapping->savepcd)
