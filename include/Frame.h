...
public:
    ...
    // Constructor for RGB-D cameras with Object Detection
    Frame(Tracking* pTracker, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    void RemoveMovingPointsWithEpiAndYOLO(std::vector<double> epiErr, const cv::Mat &ImGray, const cv::Mat &ImDepth);
